...
3. 각 이벤트의 **이벤트 라벨** 파라미터에 한글 설명이 포함되어 있어, 보고서에서 쉽게 구분 가능

### 한글 라벨 (`event_label`)

모든 커스텀 이벤트에 `event_label` 파라미터가 포함되어 있습니다. GA4 보고서에서 이벤트를 클릭하면 **이벤트 라벨** 컬럼에서 한글 설명을 확인할 수 있습니다.

| 이벤트                   | event_label 예시                                                    |
| ------------------------ | ------------------------------------------------------------------- |
| cta_click                | 히어로 섹션 사전등록 버튼 (데스크탑), 플로팅 버튼 사전등록 (모바일) |
| share                    | 플로팅 버튼 - 네이티브 공유, 엔딩 섹션 - 클립보드 복사              |
| tab_switch               | 이용방법 탭 - 배달자, 이용방법 탭 - 의뢰인                          |
| preregistration_complete | 사전등록 완료                                                       |
| preregistration_error    | 사전등록 실패                                                       |
...
// 파라미터와 함께 (event_label로 한글 설명 추가 권장)
event('이벤트_이름', {
  button_name: '예시',
  location: 'hero',
  event_label: '히어로 섹션 예시 버튼 클릭',
});
```

- 이벤트 이름: `snake_case` 권장
- `event_label`: 보고서에서 한글로 표시되므로 가독성 향상
